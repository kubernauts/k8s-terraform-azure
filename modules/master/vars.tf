...
variable "domain_name_label" {
  description = <<EOF
(mandatory) Domain name label used to generate a public DNS name for the cluster.
EOF
}
...
variable "node_count" {
  ...
  description = <<EOF
(optional) The number of created master nodes. Default: 3
EOF
...
variable "first_consecutive_ip" {
  type = "string"

  description = <<EOF
(optional) The first consecutive IP address that will be assigned to master nodes.
Leave blank to allocate the first available IP within the configured subnet.
EOF
